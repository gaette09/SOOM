...
/// ⚙️(`ProfileIdentityView`) 진입점으로 완전히 분리된 설정 화면 — 계정/기준값/공개범위/알림/앱정보.
/// 정체성(아바타/성향/대표기록/뱃지/Signature Routes)은 `ProfileIdentityView`가 담당한다
...
struct SettingsView: View {
    ...
    private var connectionsSection: some View {
        SOOMCard {
            SOOMSectionHeader("Connections", caption: "권한과 외부 서비스 연결을 관리하는 영역입니다.")

            NavigationLink {
                HealthKitSettingsViewContainer()
            } label: {
                ProfileConnectionRow(connection: ProfileIdentitySystem.foundation.connections[0])
            }
            .buttonStyle(.plain)
