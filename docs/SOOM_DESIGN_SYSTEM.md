...
- Signature share card는 SOOM의 브랜드 언어를 짧게 남기는 story surface다. `페이스는 숨에서`, `숨부터 잡아라`, `길보다 리듬`, `함께 쌓은 리듬` 같은 1줄 footer를 작게 사용하고, 배경에는 가독성을 해치지 않는 subtle rhythm/breath pattern만 허용한다.
- 공유 카드는 emotion-first / metric-light 원칙을 유지한다. 큰 headline, 한 줄 해석, 작은 보조 정보, 작은 SOOM signature를 넘어서 데이터 표처럼 확장하지 않는다.
- route 기반 운동은 공유 카드에서 코스 라인이 명확히 보여야 한다. 특히 운동/코스 카드는 route line을 primary visual로 다룬다.
- `투명` export는 배경, 카드 surface, 테두리, top metadata, privacy label, checkerboard, decorative rhythm pattern을 제거하고 route/text/signature만 남기는 overlay asset이어야 한다. checkerboard와 `투명` badge는 composer preview 전용이다.
...
- 공유 카드는 9:16 비율을 기본 export 기준으로 사용한다. Instagram Story와 세로 저장 이미지를 우선하며, 기존 피드형 공유보다 story-first export를 기준으로 한다.
...
### Share Card Option / Target Foundation

- Share UI는 기술적인 type 선택이 아니라 `운동 카드`, `컨디션 카드`, `코스 카드`, `클럽 카드`를 고르는 경험으로 보여준다.
- 각 카드 옵션은 짧은 preview summary를 함께 보여준다. 예: 운동 카드는 거리와 한 줄 리듬, 컨디션 카드는 `82 · 좋음`과 한 줄 상태, 코스 카드는 코스명과 분위기, 클럽 카드는 클럽명과 기여 흐름을 보여준다.
- 배경은 지도/사진 기반 preview와 transparent export option을 제공한다.
- Share Targets는 Instagram Story, Save Image, More만 표시한다. 직접 SNS API나 서버 링크 생성은 붙이지 않고 iOS 공유 시트를 통해 처리한다.
- Copy Link는 public URL backend와 remote publish가 생기기 전까지 UI에 노출하지 않는다.
- 컨디션 카드는 기본 private이다. 공개 카드에는 개인 컨디션, 피로, readiness, 회복 점수, 민감한 코치 문장을 넣지 않는다.
...
- 기존 공유 카드와 같은 9:16 비율과 SOOM branding을 사용해 export 품질을 일관되게 유지한다.
