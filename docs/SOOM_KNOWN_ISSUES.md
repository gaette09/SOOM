...
### SOOMFirstJourneyCard Health-Connect CTAs — Manual UI Check Not Done

Found 2026-09-10 while wiring the first 3 real CTAs onto `SOOMFirstJourneyCard` (previously fully decorative — see the "SOOMFirstJourneyCard" batches in project history). Batch A wired Activity's and Feed's "첫 운동 가져오기" to `HealthKitWorkoutImportViewContainer()` and Profile's "Health 앱 연결" to `HealthKitSettingsViewContainer()`, all via `.navigationDestination(isPresented:)`. Code was reviewed diff-by-diff and the full test suite passed (1218/1218), but a live tap-through on all three was never completed: `SOOMBottomTabBar` (RootTabView.swift) would not receive simulated taps across two different simulators (iPhone 16, iPhone 16 Pro) and multiple independently recomputed coordinates — every tap at the tab bar's visual position fell through to whatever feed content sat behind it instead of switching tabs, even though the tab bar's own SwiftUI structure (a plain `HStack` of real `Button`s with `.frame(maxWidth: .infinity)`) looks correct on inspection. This blocked reaching the Activity and Profile tabs at all in this session. Both destination screens (`HealthKitWorkoutImportViewContainer`, `HealthKitSettingsViewContainer`) are pre-existing and already reachable elsewhere (from `SettingsView`), so the risk is judged low, but the new tap targets on `SOOMFirstJourneyCard` itself were never visually confirmed to actually open them. Trigger to act: next time a UI pass touches Activity, Feed emptyState, or Profile, manually tap all three "첫 운동 가져오기"/"Health 앱 연결" rows and confirm the destination screens open.
