...
### Simulator Tab Bar Rejects Automated Taps — Blocks Manual UI Verification of All SOOMFirstJourneyCard Batches

Found 2026-09-10, recurring through 2026-09-12/13 across the entire `SOOMFirstJourneyCard` wiring effort (batches A-D — see project history; the card was previously fully decorative, all four batches gave 7 of its 9 CTA rows real actions, 2 stay intentionally decorative by product decision). Every batch's code was reviewed diff-by-diff and the full test suite passed (1218/1218 each time), but **live tap-through verification in the Simulator was never completed for anything that requires switching tabs**, because `SOOMBottomTabBar` (RootTabView.swift) consistently rejects simulated taps: every tap at the tab bar's visual position — recomputed independently, multiple times, across two different simulators (iPhone 16, iPhone 16 Pro) and two different point-scale conversions — fell through to whatever feed content sat behind it instead of switching tabs.

A focused 5-minute investigation (2026-09-12) ruled out the obvious causes without finding the real one:
- **Not a coordinate/scale error**: the exact same calculation method hit a small header icon (bookmark button, ~60pt circular target) and a nav-bar "닫기" text button precisely on the first or second try on the same screen, same session. Only the bottom tab bar specifically fails.
- **Not a z-order/zIndex issue**: no `.zIndex` calls exist anywhere in RootTabView.swift; the tab bar is the last child in its `ZStack`, so it should already be topmost for hit-testing.
- **Probably not `FloatingRecoveryCoach`**: it can be present on the Feed tab (`shouldShowFloatingCoach`), but its frame is trailing-aligned (`.frame(maxWidth: .infinity, alignment: .trailing)`), which doesn't obviously explain failures at the tab bar's center/left positions.

Root cause remains unidentified. Given the 5-minute budget, investigation was deliberately stopped rather than let it block the batch work (per explicit instruction) — every batch instead shipped on code review + full test suite alone, which is the established fallback for this exact situation.

**What has NOT been manually tap-verified as a result**, and should be checked together the next time someone is at a keyboard with a real device or a working simulator setup:
- Batch A: Activity's and Feed's "첫 운동 가져오기" → `HealthKitWorkoutImportViewContainer()`; Profile's "Health 앱 연결" → `HealthKitSettingsViewContainer()` (both pre-existing, already-reachable screens elsewhere via `SettingsView`, so risk is judged low).
- Batch B: Club's "클럽 만들기" (now reuses the already-working `onCreate` closure — very low risk); Feed's "천천히 맞는 클럽 찾기" → switches `selectedTab` to `.clubs`.
- Batch C: Feed's "추천 코스 보기" → presents Record's full-screen cover with `routeRecommendationSheet` auto-opened (this one also **fixed a separate pre-existing gap** — the sheet had no `.sheet(isPresented:)` modifier anywhere before this batch, so it was completely unreachable UI; that fix itself is unverified by tap-through too).
- Batch D: Profile's "로컬로 먼저 시작" now sets a local dismiss flag — lowest risk of all (pure `@State` toggle, no navigation), but still unverified since reaching Profile requires the same blocked tab bar.

Trigger to act: next time a UI pass touches Feed, Activity, Club, Profile, or Record, manually tap through all six wired rows above and confirm each destination/behavior actually happens. If the tab bar issue itself resurfaces, it's worth a real (non-time-boxed) investigation — possibly a physical device or a fresh simulator runtime, since two different simulator instances hit the identical symptom.

탭바 버그를 실제로 고칠 때, 같이 검토할 것 — 탭 선택 시 알약형 배경이 부드럽게 이동하고 선택된 아이콘이 하이라이트되는 모션 업그레이드(matchedGeometryEffect 또는 스프링 애니메이션으로 구현 가능, 참고 레퍼런스: Instagram류 네비게이션 탭 스타일). 버그 원인 파악하며 탭바 코드를 어차피 들여다보게 될 테니, 그 김에 같이 작업하면 중복 작업 없이 한 번에 끝낼 수 있음. 순수 장식 항목이라 버그 수정보다 우선순위 낮음, 별도로 먼저 하지 말 것.

### Feed Silently Falls Back to Mock Data on Any Remote Fetch Error (launch blocker)

Found 2026-09-09 while doing screenshot verification for the Feed bookmark feature. `FeedDataSource.loadFeed()` wraps `remoteRepository.fetchFeed(limit:)` in a bare `do/catch` that, on ANY thrown error — network failure, an expired/invalid session, a malformed query, PostgREST not recognizing a table — silently discards the error and falls back to `FeedMockData.items` (the same hardcoded "정지환"/"도윤"/"민서" placeholder posts a fresh install shows). There is no logging, no user-facing message, nothing distinguishing "you're offline" from "everything is actually fine, enjoy these three fake workouts." The real incident that surfaced this: a real, logged-in account with 4 real posts in `feed_posts` still saw only mock data, because PostgREST's schema cache hadn't picked up the new `feed_bookmarks` table (batch A of this same feature) and every `fetchFeedPosts()` call was throwing `PGRST205`. Diagnosing this took several hours of dead-end investigation — checking RLS policies, re-running `supabase db dump` (which turned out to be unreliable for this too), suspecting a broken share-to-feed flow that was actually working fine — because the failure was completely invisible from both the UI and the CLI tooling used to inspect it. The only reason the real cause surfaced at all was adding a temporary `print()` in the catch block and reading the app's console output directly. This is dangerous in two directions at once: a user with a real, working account can be shown fabricated content indefinitely without any signal something is wrong, and a developer debugging "why don't my posts show up" has no way to tell a transient network hiccup apart from a schema/config error apart from "no real posts exist yet" — all three look identical. `loadPost(id:)` has a milder version of the same shape (`try?` swallowing the remote error before falling back to `fallbackRepository`). Trigger to act: before launch, at minimum log the swallowed error (even just to console/Crashlytics-equivalent) so a silent failure is diagnosable in minutes instead of hours; ideally distinguish "no real data yet" (empty, no error) from "the fetch actually failed" (real error) and only fall back silently for the former.
...
### AnalysisView Is Orphaned Dead Code Running On 100% Mock Data (post-launch, low priority)

Found 2026-09-13 while scoping the Growth-Comparison sections work. `AnalysisView`/`AnalysisViewContainer` (`SOOM/Features/Activity/AnalysisViewContainer.swift`) have no entry point anywhere in the app — confirmed via grep, the only references to either type are inside their own definition file. No tab, no `NavigationLink`, no button reaches this screen; a real user can never see it. It's driven entirely by `DashboardViewModel(harness: MockWorkoutHarness())`, and `MockWorkoutHarness.loadWorkouts()` (`SOOM/Harnesses/Mock/MockWorkoutHarness.swift`) returns 5 fully hardcoded `Workout` literals with fixed May 2026 dates — one entry's `source` field literally reads `"SOOM 더미 하네스"` ("SOOM dummy harness"). `WorkoutHarness` has no production implementation anywhere, only this mock one. Not a launch blocker since nothing reaches it today. Trigger to act: post-launch, either wire this screen up for real with `SwiftDataUnifiedWorkoutStore`-backed data if it's meant to ship, or delete it along with `MockWorkoutHarness`/`WorkoutHarness` if it's abandoned.
...
### GPX/TCX/FIT Parser Interface Unification (deferred, next-version candidate)
...
### Feed Comment List Doesn't Show Real Author Names

Found 2026-08-30 while building the comment-reading feature (club-challenge-progress-engine session, comment batches A-C). `FeedCommentListView` and the "댓글 쓰기" entry point on `FeedItemDetailView` are fully wired end-to-end — reading, writing, oldest-first ordering, both the normal feed-card navigation path and the notification deep-link path all work. But each comment row only ever shows "나" (if `isViewerAuthor`) or "멤버" — never a real display name. `FeedComment.authorId` is a bare `UUID` with nothing resolving it to a name, and `FeedItemDetailView`/`FeedItemDetailDestination`/`FeedNotificationDetailLoader` have no `profileFetcher` wired in at all (unlike club ranking and the notification inbox, which already resolve names via `fetchProfiles(ids:)`). Deliberately deferred as a separate small batch rather than folded into the read/write work — same `fetchProfiles(ids:)` pattern is reusable once someone wires it through.
...
### FeedSavedPostsView Reimplements Feed's Optimistic-Update Logic Independently

Found 2026-09-03 while landing the Feed bookmark feature's saved-posts list screen (batch C). `FeedSavedPostsViewModel` (`FeedSavedPostsView.swift`) duplicates `FeedViewModel`'s cheer/save/comment/delete optimistic-update logic (flip local state → call remote → revert on failure) against its own flat `items` array, rather than sharing an abstraction with `FeedViewModel`. Same category of issue as "Club's Fallback Pattern Is Reimplemented Independently From Feed" above — both work correctly today, but this is duplicated design that will silently drift if `FeedViewModel`'s logic changes (a fix or behavior change there won't propagate here). Not refactored this round — deferred. Trigger to act: any future change to `FeedViewModel`'s cheer/save/comment/delete handling should also update `FeedSavedPostsViewModel`, or this should be unified alongside/when the Club pattern above is addressed.
...
Found 2026-08-29 auditing notification/comment/club completeness. Writing a comment already worked end-to-end (`FeedCommentComposeSheet` → `FeedViewModel.postComment` → `SupabaseFeedRemoteClient.addComment` → `feed_comments`), but there was no comment list screen: the post-fetch query had no `.order()`/`.limit()`, and `FeedPostDTO` collapsed the whole array down to `microComment: comments.first?.body` with no guaranteed ordering — a post with 10 comments only ever surfaced 1. Fixed across three batches: (A) oldest-first ordering + a `.limit(200)` safety cap on the single-post query (the multi-post batch query keeps no cap, since a cap there could starve individual posts), `FeedItem.comments: [FeedComment]` now carries the full array; (B) new `FeedCommentListView` sheet plus a "댓글 N개 보기" entry point on `FeedItemDetailView`; (C) a "댓글 쓰기" entry point wired to the existing `FeedCommentComposeSheet` on both ways into the detail view (normal feed-card navigation and the notification deep-link path via `FeedNotificationDetailLoader`). Author display is a known follow-up — see "Feed Comment List Doesn't Show Real Author Names" below.
