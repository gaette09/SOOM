...
- UI에서는 `검증용`, `미리보기`, `공식 Recovery 미반영` 표현을 함께 사용해 사용자가 기본 Recovery 점수로 오해하지 않게 한다.
- preview score는 공식 Recovery score보다 낮은 시각 우선순위로 표시하며, imported workout 기준/분석 제외/중복 자동 미정리 boundary를 안내한다.


## 17. Recovery Comparison Preview

Recovery Comparison은 공식 `RecoverySummary`와 UnifiedWorkout 기반 preview `RecoverySummary`를 나란히 해석하는 interpretation layer다. 이 계층은 score/status/recommendation을 계산하거나 수정하지 않고, 두 결과의 차이를 사용자가 이해할 수 있게 설명한다.

정책:

- 공식 Recovery provider는 변경하지 않는다.
- Preview Recovery는 기본 점수보다 낮은 시각 우선순위로 둔다.
- 차이가 있어도 공식 점수가 틀렸거나 preview가 더 정확하다고 단정하지 않는다.
- 차이 원인은 imported workout 범위, 분석 제외 설정, 중복 기록 미정리 가능성처럼 검증 가능한 항목으로 안내한다.
- DeduplicationEngine은 자동 적용하지 않는다.
