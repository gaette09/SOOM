...
v1의 목적은 Growth/Recovery 해석 결과를 공유 가능한 형태로 정리하기 위한 제품/기술 방향을 고정하는 것이다.
...
- 실제 image export, iOS Share Sheet, Feed/SNS 서버 저장은 아직 구현하지 않는다.
- 공유 카드는 Workout Detail에서 미리보기로만 표시하며, 실제 공유 액션은 future work로 둔다.
