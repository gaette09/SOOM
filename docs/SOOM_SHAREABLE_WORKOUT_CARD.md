...
- `ShareableWeeklyProgressCardModel`이 공유 카드에 필요한 주간 요약 텍스트와 지표만 보관한다.
...
- Weekly Progress Share Card의 현재 visibility 표시는 View에서 “나만 보기”로 고정한다.

TODO:

- 향후 Weekly Progress Card에 팔로워/공개 공유 범위를 추가할 때는 `ShareableWeeklyProgressCardModel`에도 privacy/visibility 필드를 추가해 Workout Session Summary Card와 같은 정책 구조로 맞춘다.
