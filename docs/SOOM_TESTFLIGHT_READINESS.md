...
- Current-location QA should confirm the recenter button remains neutral while the actual map marker uses a compact SOOM purple dot/ring/halo with a small 48-52pt max pulse; Reduce Motion should leave the marker static and fallback/sample markers should not use a strong pulse.
- READY visual QA should confirm the button remains primary without hiding too much of the lower map, stays around 78-82pt, and that the bottom dim gradient is barely visible until the long-press selection state.
