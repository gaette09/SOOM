...
-- Scope: feed posts, media, reactions, comments.
-- Depends on profiles_v1.sql for author display_name/handle (apply that
-- file first — feed_posts.user_id alone has no display name to show).
-- Deferred: follows graph (see follows_v1.sql, not yet written), photo
-- storage upload, pagination cursor.
...
-- Matches club_foundation_v1.sql's convention of indexing every foreign
-- key column used by RLS EXISTS subqueries and by the eventual feed fetch
-- query (filter by user_id/visibility, sort by created_at).
