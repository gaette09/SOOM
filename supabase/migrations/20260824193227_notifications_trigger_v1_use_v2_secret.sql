...
--
-- Secret name carries a _v2 suffix: the original value was lost after
-- provisioning (Vault/secrets are write-only, no way to re-read a value
-- once set) while verifying with a direct curl call, so a replacement
-- was provisioned under a new name rather than guessing the old one.
--
...
begin
  ...
  where name = 'notify_feed_interaction_webhook_secret_v2';
...
-- 4. select vault.delete_secret(id) from vault.decrypted_secrets where name = 'notify_feed_interaction_webhook_secret_v2';
