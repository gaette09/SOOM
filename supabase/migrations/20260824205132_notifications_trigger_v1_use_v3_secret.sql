...
--
-- Secret name carries a _v3 suffix (history: _v1 lost during initial
-- curl verification, _v2 rotated out after the original APNs .p8 key
-- was reissued on 2026-08-24 — Vault/secrets are write-only, so each
-- rotation goes under a new name rather than trying to overwrite the
-- old value in place).
--
...
begin
  ...
  where name = 'notify_feed_interaction_webhook_secret_v3';
...
-- 4. select vault.delete_secret(id) from vault.decrypted_secrets where name = 'notify_feed_interaction_webhook_secret_v3';
